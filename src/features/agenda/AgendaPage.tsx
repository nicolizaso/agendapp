  ...
  return (
    <div className="space-y-6 pb-24">
      <header>
        ...
        <p className="mt-1 text-sm text-ink-400">Reservá tus días de entrenamiento y planificá la progresión.</p>
      </header>
...
function SessionsView() {
  ...
  const upcoming = sessions.filter((session) => new Date(session.date) >= today);
  const past = sessions.filter((session) => new Date(session.date) < today).slice(-5).reverse();
  ...
  const handleStart = (session: ScheduledSession) => {
    ...
    } else if (session.planId && typeof session.weekIndex === 'number') {
      loadPlanWeekIntoWorkout(session.planId, session.weekIndex);
    }
  };
  ...
  const describe = (session: ScheduledSession) => {
    ...
    if (session.planId) {
      ...
      return {
        label: plan ? `${plan.name} · Semana ${(session.weekIndex ?? 0) + 1}` : 'Plan eliminado',
    ...
    <div className="space-y-6">
      ...
      {upcoming.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-card border border-ink-800 bg-ink-900/40 px-6 py-16 text-center">
          ...
          <p className="mt-2 max-w-xs text-sm leading-relaxed text-ink-400">
            Elegí un día, un horario y qué rutina o plan vas a entrenar.
          </p>
        </div>
      ) : (
        <ul className="space-y-3">
          {upcoming.map((session) => {
            ...
            const isToday = new Date(session.date).getTime() === today.getTime();
              ...
              >
                <div className="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-xl border border-ink-800 bg-ink-950 text-ink-200">
                  <span className="text-[10px] font-bold uppercase text-ember-400">
                    {new Date(session.date).toLocaleDateString('es-AR', { month: 'short' })}
                  </span>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-ink-500">
                    {isToday ? 'Hoy' : new Date(session.date).toLocaleDateString('es-AR', { weekday: 'long' })} ·{' '}
                    {session.time}
      ...
      {past.length > 0 && (
        <section className="space-y-3">
          <h2 className="font-heading text-base font-bold text-ink-100">Turnos pasados</h2>
          <ul className="space-y-2">
            {past.map((session) => {
              ...
              return (
                <li
                  key={session.id}
                  className="flex items-center justify-between gap-3 rounded-2xl border border-ink-800/60 bg-ink-900/40 p-3 text-sm"
                >
                  <span className="min-w-0 truncate text-ink-400">{info.label}</span>
